...
- 前端上传字段名固定为 `image`
- 前端文件选择器必须设置 `accept="image/*"`
- 前端提交前必须做一次轻量校验：
  - 已选择文件
  - `file.type` 以 `image/` 开头，或扩展名属于允许图片集合
- 后端仍作为最终校验方，对非法文件统一返回 `422`
...
`SessionSnapshot` 必须从“对前端暴露路径字段”切换为“只暴露可展示但不泄露服务端路径的字段”。
...
- 首次创建成功返回的 `SessionSnapshot.version` 固定为 `1`
- `result.html` 的文件内容必须与首次 `SessionSnapshot.current_html` 字节级一致
- `metadata.json.initial_version` 必须等于首次 `SessionSnapshot.version`
- `metadata.json.form_json` 与 `metadata.json.change_summary` 必须分别与首次快照中的 `current_form_json`、`summary` 对齐
...
- OCR 服务不可用
- OCR 输出格式异常
- 原图、`ocr_raw.json` 或 `ocr_compact.json` 写入失败
...
- `422` 只用于上传输入错误
- OCR、模型请求失败、模型输出结构非法都视为服务端初始化失败，统一返回 `500`
...
- 后端内部保留调试路径
- 在这一阶段先把四个 `SessionSnapshot` 接口的返回模型与测试口径统一

### 13.4 第四步：更新会话创建接口与前端 API
...
- 文本路径输入改成文件选择器
- 使用 `FormData` 创建会话
- 增加 `accept="image/*"` 与前端轻量校验
- 增加创建中的状态提示与错误提示

迁移约束：

- 第 13.3、13.4、13.5 三步按单分支原子落地
- 不要求这三步之间的中间提交保持前后端联调可运行
- 最终提交状态必须同时满足：
  - 创建接口已切换为上传文件
  - 前端创建区已切换为文件选择器
  - 四个 `SessionSnapshot` 接口已统一移除路径字段
...
- 调试产物写入失败时稳定返回“初始化失败：调试产物写入未完成”
- 首次创建成功时，`result.html`、`metadata.json` 与首个 `SessionSnapshot` 满足一致性不变量

### 14.3 前端组件测试

- 未选文件时按钮禁用
- 选择文件后可提交
- 文件选择器带有 `accept="image/*"`
- 非图片文件在前端直接拦截，不发请求
- 创建会话使用 `FormData`
- 错误信息正确渲染
